...
## Keep only the observations from Vehicle related sources in Baltimore(MA)
NEI_SCC_Vehicles<-subset(NEI_SCC,grepl("Vehicle",NEI_SCC$EI.Sector, ignore.case = TRUE)&fips == "24510")

## Sum up the emissions by year
NEIVehiclesSumEmissionsByYear <- ddply(NEI_SCC_Vehicles, .(year), summarise, sumEmissions = sum(Emissions, na.rm = TRUE))
...
ggplot(NEIVehiclesSumEmissionsByYear, aes(year, sumEmissions))+ ggtitle("Emissions from vehicle sources in Baltimore")+theme(plot.title = element_text(face="bold"))+geom_point()+geom_line()
